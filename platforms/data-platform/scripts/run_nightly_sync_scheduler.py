...
def main() -> int:
    parser = argparse.ArgumentParser(
        description='Build a governed nightly sync scheduler snapshot and dispatch plan.'
    )
    ...
    (output_dir / 'scheduler-snapshot.json').write_text(
        json.dumps(snapshot, ensure_ascii=False, indent=2),
        encoding='utf-8',
    )
    (output_dir / 'dispatch-plan.json').write_text(
        json.dumps(snapshot['dispatch_plan'], ensure_ascii=False, indent=2),
        encoding='utf-8',
    )
    (output_dir / 'cursor-ledger.json').write_text(
        json.dumps(snapshot['cursor_ledger'], ensure_ascii=False, indent=2),
        encoding='utf-8',
    )
    return 0

...
def capability_dependency_entry(capability_id: str, *, data_platform_root: Path = DATA_PLATFORM_ROOT) -> dict[str, Any]:
    ...
    if 'endpoint_contract_ids' not in normalized:
        normalized['endpoint_contract_ids'] = list(normalized.get('required_endpoint_contract_ids', []))
    if 'dependency_capability_ids' not in normalized:
        normalized['dependency_capability_ids'] = []
    return normalized

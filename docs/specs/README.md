# Specs

本目录存放经过确认的正式方案文档。

当前：

- `data-platform/`：Navly 数据中台总方案
- `navly-v1/`：Navly_v1 正式版本方案
  - `navly-v1/shared-contracts/`：Navly_v1 公共契约层专项方案包
  - `navly-v1/data-platform/`：Navly_v1 数据中台专项方案包
  - `navly-v1/auth-kernel/`：Navly_v1 权限与会话绑定内核专项方案包
  - `navly-v1/openclaw-host-bridge/`：Navly_v1 OpenClaw 宿主桥接层专项方案包
  - `navly-v1/runtime/`：Navly_v1 thin runtime shell 专项方案包
  - `navly-v1/verification/`：Navly_v1 verification / regression baseline 方案包
  - `navly-v1/2026-04-12-navly-v1-phase-1-closeout-spec.md`：Navly_v1 phase-1 closeout 正式规范
  - `navly-v1/2026-04-06-navly-v1-upstream-integration-policy.md`：各 upstream 的统一采用方式
  - `navly-v1/2026-04-06-navly-v1-implementation-kickoff.md`：implementation 启动与首批 PR 基线

当前目录：

- `data-platform/`：数据中台专项方案
- `navly-v1/`：Navly_v1 正式方案、公共契约与模块专项文档入口
